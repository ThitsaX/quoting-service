...
async function httpRequest (opts, fspiopSource) {
  ...
  try {
    const __axiosStart = Date.now()
    res = await axiosInstance.request(opts)
    log.info(`perf: __axiosStart HTTP call took ${Date.now() - __axiosStart}ms`, { url: opts.url })

    body = await res.data
    log.info(`perf: __axiosStart res.data HTTP call took ${Date.now() - __axiosStart}ms`, { url: opts.url })
